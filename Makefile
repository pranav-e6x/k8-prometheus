...
gencerts:
	...
	@echo '{"CN":"'$(SERVICE_NAME)'","hosts":[$(ALT_NAMES)],"key":{"algo":"rsa","size":2048}}' | docker run  -v ${HOME}:${HOME} -v ${PWD}/metrics-ca.key:/workdir/metrics-ca.key -v ${PWD}/metrics-ca.crt:/workdir/metrics-ca.crt -v ${PWD}/metrics-ca-config.json:/workdir/metrics-ca-config.json -i cfssl/cfssl gencert -ca=metrics-ca.crt -ca-key=metrics-ca.key -config=metrics-ca-config.json - | docker run --entrypoint=cfssljson -v ${HOME}:${HOME} -v ${PWD}/output:/workdir/output -i cfssl/cfssl -bare output/apiserver
